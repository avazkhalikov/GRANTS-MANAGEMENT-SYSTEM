...
namespace BOTAMVC3.Controllers
{
       ...
       // public IUserSession session;
        ...
        {          
           ...
           // if(Request.Form["SearchString"].ToString().Contains(
            ...
            try
            {
                ViewData["Results"] = propInfo.OrderByDescending(x =>Convert.ToInt32(x.Project.Label.ToString().Split('-').ElementAt(2))).ToList();
            }
            catch
            {
                ViewData["Results"] = propInfo.ToList(); 
            }
 ...
 //           lst.Insert(0, new SelectListItem { Text = "Not Selected", Value = "0" });
   ...
   //         ViewData["OrganizationList"] = lst; 
        ...
        [AcceptVerbs(HttpVerbs.Post)]
        public ActionResult TestAutoComplete(string SelectedOrgId)
        {
            return View();
        }


        public JsonResult ProjLocs(string term)
        {
            IEnumerable<ProjLocationList> ResultList = projservice.ProjLocationList();
            
            var test = ResultList.ToList(); 

            if (!string.IsNullOrEmpty(term) && ResultList.Any())
            {
                     var data = ResultList
                ...
                if (!data.Any() && term.Length > 3)
                //starts with failed. and user already typed in more then 3 symbols.  
                {
                    //we search more general with container.
                         data = ResultList
        ...
        public JsonResult Orgs(string term)
        {            
            IEnumerable<General> ResultOrgList = projservice.GetAllOrganizationList();
           
            if (!string.IsNullOrEmpty(term))
            {
                var data = ResultOrgList
                    .Where(r => r.NameRu.StartsWith(term))
                    .Select(r => new {r.OrgID, value = r.NameRu})
                    .Take(10)
                    .ToArray();
                    ...
                    //starts with failed. and user already typed in more then 3 symbols.  
                {
                    //we search more general with container.
                    data = ResultOrgList
                        .Where(r => r.NameRu.Contains(term))
                        .Select(r => new {r.OrgID, value = r.NameRu})
                        .Take(10)
                        .ToArray();
                }
